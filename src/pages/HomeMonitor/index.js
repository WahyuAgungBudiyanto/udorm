...
import { BackHome, NotifIcon, mainBackground, LogoAbsentBtn, HistoryIcon} from '../../assets/images';
...
const HomeMonitor = ({navigation}) => {
  ...
  const [sheetDBpreview, setsheetDBpreview] = useState('');

  const fetchSheetDBpreview = async () => {
    try {
      const sheetDBpreviewRef = r(
        db,
        `Monitor/80cKQ088SPQhmJJKCr2ANsPe5dv2/sheetDBpreview`,
      );
      onValue(sheetDBpreviewRef, snapshot => {
        if (snapshot.exists()) {
          setsheetDBpreview(snapshot.val());
        } else {
          console.log('No SheetDB endpoint available');
        }
      });
    } catch (error) {
      console.error('There was a problem fetching the endpoint:', error);
    }
  };
  ...
  const goProfile = () => {
   ...
   const goHistory = () => {
     Linking.openURL(sheetDBpreview);
   };
