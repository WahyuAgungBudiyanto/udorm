...
const Approve = ({navigation}) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
        ...
        const snapshot = await get(studentsRef);
          if (snapshot.exists()) {
            ...
            if (a.approve === 'pending' && b.approve !== 'pending') return -1;
            if (a.approve !== 'pending' && b.approve === 'pending') return 1;
            if (a.approve === 'accepted' && b.approve === 'denied') return -1;
            if (a.approve === 'denied' && b.approve === 'accepted') return 1;
            return 0;
            });
    ...
    const handleSaveApprove = async (newStatus) => {
        if (selectedStudent) {
            const db = getDatabase();
            const studentsRef = ref(db, 'Student');
            ...
            if (snapshot.exists()) {
                ...
                await update(studentRef, {approve: newStatus});
  ...
  const handleRowClick = ({Email, Name, approve, ...rest}) => {
    ...
    <PaperProvider theme={theme}>
      <View style={styles.container}>
        ...
        <TextInput
          ...
          label="Search by No. Regis, Name and Approve"
          value={searchText}
        ...
        <ScrollView>
          <DataTable style={styles.dataTable}>
            <DataTable.Header style={styles.header}>
              ...
              <DataTable.Title style={styles.headerTitle}>Name</DataTable.Title>
          ...
          <View style={styles.centeredView}>
            <View style={styles.modalView}>
              <Text style={styles.modalText}>
                {selectedStudent
                  ? `${selectedStudent.Name} (${selectedStudent.Email}@student.unklab.ac.id)`
                  : ''}
              </Text>
